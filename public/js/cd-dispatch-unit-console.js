...
;(function () {
  ...
  var state = { userId: null, unit: null, tab: 'status' };
  ...
  function statusTabHtml(u) {
    ...
    var currentId = u && u.tenCode && (u.tenCode.id || u.tenCode._id) || '';
    if (!codes.length) {
    ...
    var grid = '<div class="cd-unit-console-codes">' + codes.map(function (c) {
      var id = c._id || c.id;
    ...
    return (
      '<p class="cd-unit-console-tab-hint">Click a code to set this unit\'s status. The change broadcasts to every open dashboard in your community.</p>' +
      grid
    );
  }

  function wireStatusTab() {
    $('#cd-unit-console-body').off('click.cdUnitConsoleStatus').on('click.cdUnitConsoleStatus', '.cd-unit-console-code-btn', function () {
  ...
  function setUnitStatus(tenCodeId) {
    ...
    $.ajax({
      ...
      data: JSON.stringify({ tenCodeId: tenCodeId }),
    }).done(function () {
