...
(function () {
  ...
  var FIELDS = {"announcement": [{"name": "title", "label": "Its title"}, {"name": "content", "label": "What it says"}], "announcement_comment": [{"name": "content", "label": "What it says"}], "civilian": [{"name": "firstName", "label": "Their first name"}, {"name": "lastName", "label": "Their last name"}, {"name": "image", "label": "Their photo"}], "community_event": [{"name": "title", "label": "Its title"}, {"name": "description", "label": "Its description"}, {"name": "image", "label": "Its image"}], "feature_request": [{"name": "title", "label": "Its title"}, {"name": "description", "label": "What it says"}, {"name": "imageUrls", "label": "Its images"}], "feature_request_comment": [{"name": "content", "label": "What it says"}, {"name": "imageUrls", "label": "Its images"}], "firearm": [{"name": "name", "label": "Its name"}, {"name": "image", "label": "Its photo"}], "user_profile": [{"name": "username", "label": "Their username"}, {"name": "name", "label": "Their display name"}, {"name": "profilePicture", "label": "Their profile picture"}, {"name": "backgroundImage", "label": "Their background image"}], "vehicle": [{"name": "plate", "label": "Its plate"}, {"name": "model", "label": "Its model"}, {"name": "image", "label": "Its photo"}]};
  ...
  /* A report control for one roleplay record, for the CAD search cards.
   *
   * Built here rather than in each of the three search scripts so the wording
   * stays in one place. "Report Stolen" already means something in the CAD and
   * it is roleplay, so this says plainly who it goes to.
   *
   * Returns nothing at all for your own record: there is no sense in
   * reporting a name you wrote yourself.
   */
  window.reportRecordButtonHTML = function (kind, id, ownerId) {
    if (!FIELDS[kind] || !id) return '';
    var me = (window.ddConfig && window.ddConfig.userId) ||
      (window.dbUser && window.dbUser._id) || '';
    if (me && ownerId && String(me) === String(ownerId)) return '';
    var communityId = (window.ddConfig && window.ddConfig.communityId) || window.communityId || '';
    return '<button type="button" class="rcx-record-btn" title="Report this to LPC staff"' +
      ' onclick="event.stopPropagation();reportContent({kind:\'' + esc(kind) + '\',id:\'' + esc(id) +
      '\',communityId:\'' + esc(communityId) + '\'})">' +
      '<i class="fa fa-flag"></i> Report to LPC staff</button>';
  };
